...
public class PhotoView extends AppCompatImageView {
    ...
    public void setUpMaxScale(float mUpMaxScale) {
        this.mUpMaxScale = mUpMaxScale;
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        if (isEnable) {
            ...
            getParent().requestDisallowInterceptTouchEvent(hasMultiTouch || isZoomUp());
            return true;
        } else {
    ...
    private class Transform implements Runnable {
        ...
        void withClip(float fromX, float fromY, float deltaX, float deltaY, int d, ClipCalculate c) {
            mClipScroller.startScroll((int) (fromX * 10000), (int) (fromY * 10000),
        ...
        void withFling(float velocityX, float velocityY) {
            ...
            int distanceX =
                    (int) (velocityX > 0 ? Math.abs(mImgRect.left) : mImgRect.right - mWidgetRect.right);
            ...
            int distanceY =
                    (int) (velocityY > 0 ? Math.abs(mImgRect.top) : mImgRect.bottom - mWidgetRect.bottom);
            ...
            mFlingScroller.fling(mLastFlingX, mLastFlingY, (int) velocityX, (int) velocityY, minX, maxX,
                    minY, maxY, Math.abs(overX) < MAX_FLING_OVER_SCROLL * 2 ? 0 : MAX_FLING_OVER_SCROLL,
    ...
    public Info getInfo() {
        ...
        rect.set(p[0] + mImgRect.left, p[1] + mImgRect.top, p[0] + mImgRect.right,
                p[1] + mImgRect.bottom);
        return new Info(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter, mScale, mDegrees,
                mScaleType);
    }

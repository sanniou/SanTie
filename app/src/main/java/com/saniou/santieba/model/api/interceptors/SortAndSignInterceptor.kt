...
import com.saniou.santieba.kts.containsEncodedName
import com.saniou.santieba.kts.sortedEncodedRaw
import com.saniou.santieba.kts.sortedRaw
import com.saniou.santieba.kts.toMD5
import com.saniou.santieba.model.api.reqeust.Param
...
class SortAndSignInterceptor(private val appSecret: String) : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        request = when {
            url.queryParameter("BDUSS") != null && url.queryParameter(Param.SIGN) == null -> {
                ...
                request.newBuilder()
                    .url(
                        url.newBuilder()
                            .encodedQuery(
                                "$sortedEncodedQuery&${Param.SIGN}=${calculateSign(
                                    sortedQuery,
                                    appSecret
                                )}"
                            )
            ...
            //在 FormBody 里
            body is FormBody && body.containsEncodedName("BDUSS") && !body.containsEncodedName(
                Param.SIGN) -> {
